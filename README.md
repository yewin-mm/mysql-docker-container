...
* `--restart` need to add policy, there are 4 policy, `no` (default, if no set), `on-failure`, `always`, `unless-stopped`.
	* I used `unless-stopped` policy because I don't want auto restart my MySQL container after docker deamon (docker desktop) was started on both manually stopped condition and docker deamon stopped condition. You can use `always` policy if you want auto restart after docker desktop was started or even your application was runtime failed, it will always auto restart (it will cause high load in memory). for more details about restart policy, you can check in [Docker restart policy](https://docs.docker.com/config/containers/start-containers-automatically/).
